...
def connect_db():
    sql = sqlite3.connect('/mnt/c/sql/food_log.db')
...
def index():
    ...
    if request.method == 'POST':
        date = request.form['date'] # date is YYY-MM-DD the date form of Chrom
        # converting the date from Chrom form to YYYYMMDD without dashes 
        dt = datetime.strptime(date, '%Y-%m-%d') # dt gonna be (YYYY,MM,DD,H,M)
        database_date = datetime.strftime(dt,'%Y%m%d') #from here final_database_date will be YYYYMMDD
        db.execute('insert into log_date (entry_date) values (?)', [database_date])
        db.commit()
    cur = db.execute('select entry_date from log_date order by entry_date desc') #show the dates by order
...
@app.route('/view/<date>', methods = ['GET','POST']) #date is gonna be YYYYMMDD
def view(date):
    db = get_db()
...
def food():
    ...
    if request.method == 'POST':
        ...
        carbohydrates = int(request.form['protein'])
